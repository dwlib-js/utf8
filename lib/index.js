...
const IsArrayBuffer = require('@dwlib/abstract/IsArrayBuffer');
...
if (TextDecoder) {
  ...
  const REPLACEMENT_CHAR = StringFromCodePoint(0xfffd);
...
const decode = (buffer, options) => {
  if (!IsArrayBuffer(buffer) && !ArrayBufferIsView(buffer)) {
